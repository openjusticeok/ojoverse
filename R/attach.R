...
ojoverse_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core ojoverse packages"),
    right = paste0("ojoverse ", package_version_h("ojoverse"))
  )
